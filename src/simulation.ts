...
export function evaluateCircuit(state: EditorState): void {
  ...
  for (const wire of state.wires) {
    const from = compIndex.get(wire.fromComponentId);
    const to = compIndex.get(wire.toComponentId);
  ...
  for (const idx of order) {
    ...
    const inputValues = inputPins.map((_pin, pinIdx) => {
      ...
      const wire = state.wires.find(
        (w) => w.toComponentId === comp.id && w.toPinIndex === absPinIndex,
      );
      if (!wire) return 0;
      // Get the source component's pinValues for the output pin
      const srcComp = state.components.find((c) => c.id === wire.fromComponentId);
      if (!srcComp) return 0;
      const pinValues = srcComp.state.pinValues as number[] | undefined;
      return pinValues?.[wire.fromPinIndex] ?? 0;
    });
