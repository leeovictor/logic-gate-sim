...
export function splitWireAtJunction(
  ...
  state.wireSegments.push({
    ...
    to: junctionEndpoint,
    waypoints:
      waypointsBeforeJunction.length > 0 ? waypointsBeforeJunction : undefined,
  });
  state.wireSegments.push({
    id: `wire-${state._nextWireId++}`,
    ...
    to: seg.to,
    waypoints:
      waypointsAfterJunction.length > 0 ? waypointsAfterJunction : undefined,
  });
  return true;
}
...
export function removeWireSegment(state: EditorState, segmentId: string): void {
  ...
  for (const seg of state.wireSegments) {
    if (seg.from.type === "junction")
      referencedJunctionIds.add(seg.from.junctionId);
    if (seg.to.type === "junction")
      referencedJunctionIds.add(seg.to.junctionId);
  }
  state.junctions = state.junctions.filter((j) =>
    referencedJunctionIds.has(j.id),
  );
}

export function removeSegmentsForComponent(
  state: EditorState,
  componentId: string,
): void {
  state.wireSegments = state.wireSegments.filter((w) => {
    const fromConnected =
      w.from.type === "pin" && w.from.componentId === componentId;
