...
describe("Step Simulation", () => {
  it("step propaga sinal: Switch → NOT → Light", () => {
    ...
    addWire(
      state,
      { type: "pin", componentId: not.id, pinIndex: 1 },
      { type: "pin", componentId: light.id, pinIndex: 0 }
    );

    // Step 1: switch output propaga
    stepCircuit(state);
    expect(state.stepSimulation.stepCount).toBe(1);

    // Step 2: NOT recebe input e avalia
    stepCircuit(state);
    expect(state.stepSimulation.stepCount).toBe(2);

    // Step 3: NOT output propaga e light recebe
    stepCircuit(state);
    expect(state.stepSimulation.stepCount).toBe(3);

    // Step 4: estabilidade ou valores finalizados
    stepCircuit(state);
    // NOT(1) = 0 → light should eventually be off
  ...
  it("ciclo NOT→NOT oscila sem erro", () => {
    ...
    addWire(
      state,
      { type: "pin", componentId: not1.id, pinIndex: 1 },
      { type: "pin", componentId: not2.id, pinIndex: 0 }
    );
    addWire(
      state,
      { type: "pin", componentId: not2.id, pinIndex: 1 },
    ...
    const not1Pins = not1.state.pinValues as (0 | 1 | "E")[];
    const not2Pins = not2.state.pinValues as (0 | 1 | "E")[];
  ...
  it("AND gate recebe sinais em múltiplos steps", () => {
    const state = createEditorState();
    state.simulationEnabled = true;
    state.simulationMode = "step";
    ...
    const gate = addComponent(state, "and-gate", { x: 200, y: 0 });
    const light = addComponent(state, "light", { x: 300, y: 0 });
    ...
    // First few steps to propagate signals
    for (let i = 0; i < 3; i++) {
      stepCircuit(state);
    }

    // Both inputs ON → AND output should be ON → light ON
