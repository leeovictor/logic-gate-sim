...
import { addComponent, addWire, createEditorState } from "@/state";
...
describe("buildNets", () => {
  ...
  it("creates single net for two components connected by wire", () => {
    ...
    const connectedNet = nets.find((n) =>
      n.pinReferences.some((p) => p.componentId === sw.id && p.pinIndex === 0),
    );
    expect(connectedNet).toBeDefined();
    expect(
      connectedNet?.pinReferences.some(
        (p) => p.componentId === light.id && p.pinIndex === 0,
      ),
    ).toBe(true);
  });
  ...
  it("creates separate nets for disconnected components", () => {
    ...
    const _sw1Output = `pin:${sw1.id}:0`;
    const _sw2Output = `pin:${sw2.id}:0`;
    const net1 = nets.find((n) =>
      n.pinReferences.some((p) => p.componentId === sw1.id && p.pinIndex === 0),
    );
    const net2 = nets.find((n) =>
      n.pinReferences.some((p) => p.componentId === sw2.id && p.pinIndex === 0),
    );
  ...
  it("creates single net for three components in series", () => {
    ...
    const net1 = nets.find((n) =>
      n.pinReferences.some((p) => p.componentId === sw.id),
    );
    expect(net1).toBeDefined();
    expect(
      net1?.pinReferences.some(
        (p) => p.componentId === gate.id && p.pinIndex === 0,
      ),
    ).toBe(true);
  });
...
describe("resolveNetSignal", () => {
  it("returns 0 for net with no output drivers", () => {
    ...
    addWire(
      state,
      { type: "pin", componentId: light1.id, pinIndex: 0 },
      { type: "pin", componentId: light2.id, pinIndex: 0 },
    );
  ...
  it("returns error (E) for multiple drivers with conflicting values", () => {
    ...
    const nets = buildNets(state);
    const net = nets.find((n) =>
      n.pinReferences.some((p) => p.componentId === light.id),
    )!;
    const signal = resolveNetSignal(net, state);
...
describe("net and error propagation through gates", () => {
  it("AND gate with error input produces error output", () => {
    ...
    const outputNet = nets.find((n) =>
      n.pinReferences.some(
        (p) => p.componentId === gate.id && p.pinIndex === 2,
      ),
    )!;
